...
(function () {
  ...
  (async function loadShaders() {
    ...
    /* Mobile keeps LIVE shaders. We previously hard-disabled WebGL below 768px
       to dodge context exhaustion, but the viewport lazy-loader further down
       already caps the number of SIMULTANEOUS contexts — off-screen edges
       suspend and release their context, so a phone only ever holds the handful
       of on-screen shaders (hero grain + subject-badge ring + the visible gold
       button edge), well within the ~8 budget. If WebGL or the module genuinely
       fails, the catch block below still degrades every host to the CSS
       foil/gradient fallback. */
    try {
      ...
      window.CSAShaders = { ok: false, error: String(e) };
      return;
    }
    ...
    function wireMetalDefaults(root) {
      ...
      node.querySelectorAll('.csa-btn-pill,.csa-btn-fill,.csa-gold-ring,.btn--gold-pill').forEach(function (el) {
        ...
        if (autoMetalCount >= AUTO_METAL_MAX) return; // beyond budget: keep CSS foil
