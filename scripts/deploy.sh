...
if [ "$REUSE_ENV" -eq 0 ]; then
  ...
  echo; echo "${c_bold}Database (self-hosted Supabase Postgres — host = supabase-db, port 5432, NOT the 6543 pooler)${c_rst}"
  echo "  ${c_dim}The app joins the Supabase docker network; host = the Postgres CONTAINER name.${c_rst}"
  echo "  ${c_dim}Standard self-hosted Supabase names it 'supabase-db' (verify: docker ps --filter name=supabase).${c_rst}"
  echo "  ${c_dim}PASSWORD = POSTGRES_PASSWORD from the Supabase stack's own .env.${c_rst}"
  ask_secret DATABASE_URI "DATABASE_URI (postgres://postgres:PASSWORD@supabase-db:5432/postgres)"
